...
HardwareMonitor::HardwareMonitor(QObject *parent): QObject(parent){
    ...
    sensorBridgePath = QCoreApplication::applicationDirPath() + "/SensorBridge.exe";
...
HardwareMonitor::SystemStats HardwareMonitor::getSystemStats()
{
    ...
    QProcess process;
    process.start(sensorBridgePath);

    if (!process.waitForFinished(5000)) {  // ← increased from 3000
        return stats;
    }
